...
window.TOPBRS_APP_VERSION = 'firestore-api-url-fix-2';
...
async function getApiBaseUrl(){
  try{
    const mod = await import('./api-config.js?v=firestore-api-url-fix-2');
...
async function fetchClanFromApi(tag){
  ...
  for(const url of urls){
    try{
      ...
      const foundClan = data?.clan || data?.data?.clan || data?.data || ((data?.tag && data?.name) ? data : null);

      if(response.ok && foundClan){
        return foundClan;
      }

      lastError = new Error(data?.message || data?.reason || data?.details?.reason || 'Clã não encontrado.');
    }catch(error){
      lastError = error;
    }
  }
