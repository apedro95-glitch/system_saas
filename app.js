...
window.TOPBRS_APP_VERSION = 'signup-clan-tag-fix-1';
...
function showClanResultSheet(type, data = {}){
  ...
  const eyebrow = isSuccess ? 'CLÃ ENCONTRADO' : isRegistered ? 'CLÃ JÁ CADASTRADO' : isNotReleased ? 'ASSINATURA NÃO LIBERADA' : 'CLÃ NÃO ENCONTRADO';
  const title = isSuccess ? data.name : isRegistered ? 'Este clã já possui cadastro' : isNotReleased ? 'Clã sem liberação' : 'Clã não encontrado';
  const message = isSuccess
    ? `${data.tag} • ${data.members} membros`
    : isRegistered
      ? 'Faça login ou solicite acesso ao administrador do clã.'
      : isNotReleased
        ? 'Toque em Assine agora ou fale com o suporte para liberar o teste.'
        : 'Confira a tag e tente novamente.';

  overlay.innerHTML = `
    <div class="clan-result-sheet glass-panel ${isSuccess ? 'success' : isRegistered ? 'registered' : isNotReleased ? 'not-released' : 'error'}">
      ...
      ${!isSuccess && !isRegistered && !isNotReleased ? `<button class="primary-btn sheet-confirm error-btn" type="button" id="sheetTryAgain">Tentar novamente</button>` : ''}
    </div>
  `;
