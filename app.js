  ...
  overlay.innerHTML = `
    <div class="clan-result-sheet glass-panel ${isSuccess ? 'success' : 'error'}">
      ...
      <div class="sheet-content">
        ...
        <h3>${isSuccess ? data.name : 'Clã não encontrado'}</h3>
        <p>${isSuccess ? `${data.tag} • ${data.members} membros` : 'Confira a tag digitada e tente novamente.'}</p>
      </div>
      ${isSuccess ? `<button class="gold-btn sheet-confirm" type="button" id="sheetConfirmClan">Confirmar Clã</button>` : `<button class="primary-btn sheet-confirm error-btn" type="button" id="sheetTryAgain">Tentar novamente</button>`}
    </div>
  `;
  ...
  onboardShell(2, `
    ...
    <div class="clan-confirm-card">
      ...
      <div class="clan-stats">
        ...
        <div class="clan-stat"><span>País</span><strong class="clan-country-flag" title="${clan.location}">${clan.countryFlag || '🏳️'}</strong></div>
      </div>
    </div>
