...
window.TOPBRS_APP_VERSION = 'member-signup-lock-1';
...
function showClanResultSheet(type, data = {}){
  ...
  const eyebrow = isSuccess ? 'CLÃ ENCONTRADO' : isRegistered ? 'CLÃ JÁ CADASTRADO' : 'CLÃ NÃO ENCONTRADO';
  const title = isSuccess ? data.name : isRegistered ? 'Este clã já possui cadastro' : 'Clã não encontrado';
  const message = isSuccess
    ? `${data.tag} • ${data.members} membros`
    : isRegistered
      ? 'Faça login ou solicite acesso ao administrador do clã.'
      : 'Confira a tag e tente novamente.';

  overlay.innerHTML = `
    <div class="clan-result-sheet glass-panel ${isSuccess ? 'success' : isRegistered ? 'registered' : 'error'}">
      ...
      <div class="sheet-clan-icon real-clan-badge-sheet">
        ${isSuccess || isRegistered ? `<img src="${data.badge || 'assets/icons/clan.svg'}" alt="" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='assets/icons/clan.svg'">` : `<img src="assets/icons/clan.svg" alt="">`}
      </div>
      ...
      ${!isSuccess && !isRegistered ? `<button class="primary-btn sheet-confirm error-btn" type="button" id="sheetTryAgain">Tentar novamente</button>` : ''}
    </div>
  `;
...
async function fetchPlayerFromApi(tag){
  const endpoints = [
    `${API_BASE_URL}/api/player/${encodeURIComponent(cleanTag(tag))}`,
    `${API_BASE_URL}/api/member/${encodeURIComponent(cleanTag(tag))}`
  ];
...
function showSignupFace(){
  ...
  document.querySelector('#signupBtn')?.addEventListener('click', async ()=>{
    ...
    try{
      ...
      await registerUser({ email, senha, playerTag: normalizeClanTag(playerTag), nome, nick });
    }catch(error){
...
async function validatePlayerTagMock(){
  ...
  const selectedClan = localStorage.getItem('selectedClan') || localStorage.getItem('topbrs_clan_tag') || '';

  btn.disabled = true;
  btn.textContent = 'Validando...';
  feedback.textContent = 'Consultando jogador e pré-cadastro...';
  feedback.classList.add('loading');

  try{
    const player = await fetchPlayerFromApi(normalized);

    if(!player){
      feedback.className = 'tag-feedback error';
      feedback.textContent = 'Tag não encontrada na API do Clash.';
      btn.disabled = false;
    ...
    nickInput.value = player.name || result.member?.name || 'Jogador validado';
    tagInput.dataset.validated = 'true';
    tagInput.dataset.nick = nickInput.value;

    feedback.className = 'tag-feedback success';
    feedback.textContent = 'Tag validada e pré-cadastro encontrado.';
