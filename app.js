...
const API_BASE_URL = localStorage.getItem('TOPBRS_API_URL') || 'https://delight-rom-bookstore-relaxation.trycloudflare.com';
const API_BASE_CANDIDATES = [
  API_BASE_URL,
  localStorage.getItem('TOPBRS_API_URL'),
  localStorage.getItem('topbrs_api_url')
].filter(Boolean);
window.TOPBRS_APP_VERSION = 'onboarding-search-popup-fix-1';
...
async function fetchClanFromApi(tag){
  ...
  for(const url of urls){
    try{
      ...
      lastError = new Error(data?.message || data?.details?.reason || 'Clã não encontrado.');
    }catch(error){
      lastError = error;
    }
  }
