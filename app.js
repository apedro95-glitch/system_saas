...
window.TOPBRS_APP_VERSION = 'i18n-stage4-final';
...
function showClanResultSheet(type, data = {}){
  ...
  if(!overlay){
    ...
    overlay.className = 'clan-result-sheet-overlay auth-result-sheet-overlay';
...
function showLoginFace(){
  ...
  authBack?.setAttribute('data-auth-mode','login');
  if(!content || !flip){
...
function showSignupFace(){
  ...
  const flip = document.querySelector('#authFlip');
  const authBack = document.querySelector('#authBack');
  authBack?.setAttribute('data-auth-mode','signup');
  if(!content || !flip){
    console.warn('Auth back container ausente. Recriando tela inicial.');
    renderSearch();
  ...
  content.innerHTML = `
    ...
    <label class="field auth-field">
      <span data-i18n="auth.clanTag">Tag do clã</span>
      <input id="signupClanTag" type="text" placeholder="#CLAN123" data-i18n-placeholder="auth.clanTagPlaceholder" autocomplete="off" />
    </label>
