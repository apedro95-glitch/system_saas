...
function renderTable(){const head=document.querySelector('#rankingTableHead'),list=document.querySelector('#rankingList'),r=rows().slice(3); if(activeTab==='donations'){head.className='ranking-table-head donations-head';head.innerHTML='<span>Pos</span><span>Membro</span><span>Env</span><span>Rec</span>';list.innerHTML=r.map((it,i)=>`<button class="ranking-row donation-row ${it.current ? 'is-current-member' : ''}"><span>${i+4}</span><strong>${it.name}${it.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong><b>${it.donSent}</b><b>${it.donReceived}</b></button>`).join('')}else{head.className='ranking-table-head';head.innerHTML='<span>Pos</span><span>Membro</span><span>Pontos</span>';list.innerHTML=r.map((it,i)=>`<button class="ranking-row ${it.current ? 'is-current-member' : ''}" data-name="${it.name}"><span>${i+4}</span><strong>${it.name}${it.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong><b>${value(it)}</b></button>`).join(''); if(activeTab==='tournament')list.querySelectorAll('.ranking-row').forEach(x=>x.onclick=()=>openDetail(x.dataset.name))}}
