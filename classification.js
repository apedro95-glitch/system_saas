...
function isCurrentSelectedPeriod(){const p=getDonationWeekContext(new Date());return selectedMonthKey()===p.monthKey && selectedWeek===p.weekKey;}
function isFutureSelectedPeriod(){const {monthKey,weekKey}=selectedPeriodKey();const now=getDonationWeekContext(new Date());return `${monthKey}_${weekKey}` > `${now.monthKey}_${now.weekKey}`;}
...
function rowsFromCurrentMembers(members, warHistory=null){
 ...
 return members.filter(m=>!m.removed).map(m=>{
  ...
  const weeklyFame=Number(wm.weeklyFame||0);
  const weeklyAttacks=Number(wm.weeklyAttacks||0);
  const warScore=Number(wm.warPoints||m.warPoints||m.generalPoints||0);
  return {name:m.name||'Membro',role:m.role||'Membro',tag:m.tag||'',avatarSrc:getAvatarForMember(m),current:isCurrentUserMember(m),general:warScore || weeklyFame || 0,tournament:Number(m.tournamentPoints||0),donSent:Number(m.donations||0),donReceived:Number(m.donationsReceived||0),weeklyFame,weeklyAttacks,weeks:{S1:Number(m.tournamentS1||0),S2:Number(m.tournamentS2||0),S3:Number(m.tournamentS3||0),S4:Number(m.tournamentS4||0)}};
 });
}
...
const current=getDonationWeekContext(new Date());
