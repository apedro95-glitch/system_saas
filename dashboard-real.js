import { loadClan, loadMembers, formatNumber, periodLabelNow, warWindowState, syncClanAndMembersFromApi } from './real-data.js';
import { syncWarSilently, loadCurrentWarForUi } from './war-logic.js';
import { isCurrentUserMember, findCurrentMemberProfile } from './identity.js';

async function hydrateDashboard(){
  const clan=await loadClan();
  const members=(await loadMembers()).filter(m=>!m.removed);
  await findCurrentMemberProfile().catch(()=>null);
  ...
  const realWarPoints=Object.values(currentWar.members||{}).reduce((sum,m)=>sum+Number(m.warPoints||0),0);
  ...
  document.querySelectorAll('[data-war-fame]').forEach(el=>el.textContent=formatNumber(realWarPoints));
  ...
  const active=[...members].sort((a,b)=>String(b.lastSeen||'').localeCompare(String(a.lastSeen||'')))[0];
  ...
  document.querySelectorAll('[data-highlight-active-name]').forEach(el=>el.textContent=active?active.name:'Em breve');
  document.querySelectorAll('[data-highlight-active-value]').forEach(el=>el.textContent=active?`${formatNumber(active.trophies||0)} troféus`:'0 ataques');
...
window.topbrsAfterRefresh=hydrateDashboard;
await hydrateDashboard();
// Primeiro mostra Firestore/cache; depois sincroniza Clash sem piscar tela.
syncClanAndMembersFromApi().then(()=>syncWarSilently()).then(()=>hydrateDashboard()).catch(()=>{});
document.querySelectorAll('.notification-row,.notification-item').forEach(row=>{ row.addEventListener('click', e=>{ row.classList.toggle('expanded'); e.stopImmediatePropagation(); }, true); });
