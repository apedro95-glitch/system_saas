...
import { syncClanAndMembersFromApi, loadMembers as loadRealMembers } from './real-data.js';
...
const MEMBERS_FALLBACK = {
  'pt-BR': {'members.title':'Membros','members.subtitle':'Lista de membros do clã.','members.search':'Buscar membro...','members.sync':'Sincronizar membros','members.member':'Membro','members.coLeader':'Co-líder','members.elder':'Ancião','members.leader':'Líder','members.count':'{{count}} membros','members.noMembers':'Nenhum membro encontrado.','members.loadError':'Não foi possível carregar membros.'},
  'en-US': {'members.title':'Members','members.subtitle':'Clan member list.','members.search':'Search member...','members.sync':'Sync members','members.member':'Member','members.coLeader':'Co-leader','members.elder':'Elder','members.leader':'Leader','members.count':'{{count}} members','members.noMembers':'No members found.','members.loadError':'Could not load members.'},
  'es-ES': {'members.title':'Miembros','members.subtitle':'Lista de miembros del clan.','members.search':'Buscar miembro...','members.sync':'Sincronizar miembros','members.member':'Miembro','members.coLeader':'Colíder','members.elder':'Veterano','members.leader':'Líder','members.count':'{{count}} membros','members.noMembers':'Nenhum membro encontrado.','members.loadError':'Não foi possível carregar membros.'}
};
...
async function loadMembersFromFirestore(){
  ...
  try{
    ...
    const rawMembers = snap.docs.length ? snap.docs.map(docSnap=>({id:docSnap.id, ...docSnap.data()})) : await loadRealMembers();

    membersData = rawMembers.map((data)=>{
      ...
      return {
        ...
        avatar: data.avatar || "",
        current: isCurrentUserMember(data),
...
/* Surgical fix: header lock */
function lockMembersHeader(){
  const h=document.querySelector('.members-header h1');
  const p=document.querySelector('.members-header p');
  const count=document.querySelector('#membersCount');
  if(h) h.textContent=tr('members.title');
...
setTimeout(lockMembersHeader, 60);
setTimeout(lockMembersHeader, 400);
window.addEventListener('topbrs:languagechange', ()=>setTimeout(lockMembersHeader, 40));
