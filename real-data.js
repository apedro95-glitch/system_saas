...
export async function fetchClanFromApi(tag){
  ...
  const res = await fetch(`${apiBaseUrl}/api/clan/${encodeURIComponent(cleanTag(tag))}`, {cache:"no-store"});
...
export async function fetchMembersFromApi(tag){
  const apiBaseUrl = await getApiBaseUrl();
  const res = await fetch(`${apiBaseUrl}/api/clan/${encodeURIComponent(cleanTag(tag))}/members`, {cache:"no-store"});
  const data = await res.json();
