...
async function saveSaasPlan(clanTag,current,plan,expiresAt,note=""){
  const clanRef = doc(db,"clans",clanTag);
  const clanSnap = await getDoc(clanRef).catch(()=>null);
  const clanData = clanSnap?.exists?.() ? (clanSnap.data() || {}) : {};
  const hasCompletedOnboarding = Boolean(clanData.onboardingComplete || clanData.ownerUid || clanData.adminUid || clanData.ownerEmail);
  const accessStatus = hasCompletedOnboarding ? "active" : "released";
  ...
  await setDoc(doc(db,"saasAccess",clanTag),{
    ...
    status: accessStatus,
    archived:false,
    allowedOnboarding: !hasCompletedOnboarding,
    onboardingComplete: hasCompletedOnboarding ? true : false,
    consumedAt: hasCompletedOnboarding ? (current.consumedAt || null) : deleteField(),
    planExpiresAt:Timestamp.fromDate(expiresAt),
    updatedAt:serverTimestamp()
  },{merge:true});

  // Antes do onboarding, o clã pode existir apenas como pré-liberação SaaS.
  // Ele NÃO pode ser marcado como active/registrado, senão a tela inicial bloqueia o onboarding.
  await setDoc(clanRef,{
    ...
    paymentProvider: current.paymentProvider || "manual",
    orderId: current.orderId || "",
    subscriptionStatus: hasCompletedOnboarding ? "active" : "released",
    active: hasCompletedOnboarding ? true : false,
    onboardingComplete: hasCompletedOnboarding ? true : false,
    archived:false,
    preOnboarding: !hasCompletedOnboarding,
    planExpiresAt:Timestamp.fromDate(expiresAt),
    updatedAt:serverTimestamp()
  },{merge:true});
}
    ...
    ${filtered.map((item,idx)=>{
      ...
      return `<article class="saas-sub-card ${isArchived ? 'archived' : ''}">
        ...
        <div class="saas-inline-actions saas-sub-actions">
          ${isArchived ? `<button onclick="restoreSaasAccess('${tag}')">Restaurar</button>` : `<button onclick="extendSaasPlan('${tag}','${id}')">Renovar</button><button class="secondary" onclick="resetSaasPlan('${tag}','${id}')">Trocar plano</button><button class="secondary" onclick="toggleSubscriptionDetails(this)">Detalhes</button><button class="danger" onclick="revokeSaasAccess('${tag}')">Revogar</button>`}
        </div>
      </article>`;
    }).join("")}
  </div>`;
