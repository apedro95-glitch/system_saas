...
function renderDashboard(){
  ...
  dash.innerHTML = `
    <div class="saas-kpi-grid">
      ...
      ${metricCard("Clãs ativos", m.active.length + m.trials.length, "⬟", "teal", "total operacional")}
    </div>
    ${renderChart()}
    ${section("Gestão de Assinaturas", `<div class="saas-mini-pills"><span>Vendas <b>${requestsCache.length}</b></span><span>Trial <b>${m.trials.length}</b></span><span>Ativos <b>${m.active.length}</b></span><span>Expirados <b>${m.expired.length}</b></span><span>Revogados <b>${accessCache.filter(i=>String(i.status||"").toLowerCase()==="blocked").length}</b></span><span>Renovações <b>${m.active.length}</b></span><span>Upgrades/Downgrades <b>${requestsCache.filter(i=>i.requestType === "planUpgrade").length}</b></span></div>`)}
