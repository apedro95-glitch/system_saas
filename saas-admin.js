...
function renderSales(){
  ...
  list.innerHTML = requestsCache.map(item=>{
    ...
    return `<article class="saas-row saas-request-row">
      <div class="saas-main-info"><strong>${item.requestType === "planUpgrade" ? "Upgrade/Renovação" : (item.name||item.clanName||item.clanTag||"Solicitação")}</strong><span>${item.clanTag||"sem tag"} • ${item.buyerEmail||item.email||"sem email"}</span><small>${item.requestType === "planUpgrade" ? `Solicitado: ${planLabel(item.plan)} ${item.price ? "• "+item.price : ""}` : "Nova venda/assinatura"}</small></div>
