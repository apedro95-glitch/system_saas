...
window.deleteSaasRequest = async function(requestId){
  if(!requestId) return;
  ...
  try{
    await deleteDoc(doc(db, "subscriptionRequests", requestId));
    await loadRequestsList();
  }catch(error){
    console.error(error);
    alert("Erro ao excluir solicitação. Verifique as regras do Firestore.");
  }
};

window.revokeSaasAccess = async function(clanTag){
  const normalizedTag = normalizeTag(clanTag);

  if(!normalizedTag) return;

  if(!confirm(`Revogar assinatura/liberação do clã ${normalizedTag}?`)){
    return;
  }

  try{
    await setDoc(doc(db, "saasAccess", normalizedTag), {
      status: "blocked",
    ...
    await setDoc(doc(db, "clans", normalizedTag), {
    ...
    console.error(error);
    alert("Erro ao revogar assinatura. Verifique as regras do Firestore.");
  }
};
      ...
      return `
        <article class="saas-row saas-request-row">
          ...
          <div class="saas-actions">
            ...
            <button class="saas-small-action danger" type="button" onclick="deleteSaasRequest('${docSnap.id}')">Excluir</button>
          </div>
        </article>
      `;
    }).join("");
...
async function loadAccessList(){
  ...
  try{
    ...
    list.innerHTML = snap.docs.map(docSnap=>{
      const item = docSnap.data() || {};
      return `
        <article class="saas-row saas-access-row">
          <div>
          ...
          <button class="saas-small-action danger" type="button" onclick="revokeSaasAccess('${item.clanTag || docSnap.id}')">Revogar</button>
        </article>
      `;
    }).join("");
  }catch(error){
