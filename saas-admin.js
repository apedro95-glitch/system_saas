  ...
  try{
    ...
    accessCache = await Promise.all(accessSnap.docs.map(async d=>{
      ...
      return {
        ...
        buyerName: access.buyerName || access.ownerName || clan.buyerName || clan.ownerName || clan.adminName || clan.leaderName || access.buyerDisplayName || clan.buyerDisplayName || "",
        buyerPhone: access.buyerPhone || access.phone || clan.buyerPhone || clan.phone || clan.whatsapp || clan.contactPhone || "",
        buyerEmail: access.buyerEmail || access.email || clan.buyerEmail || clan.email || clan.contactEmail || "",
...
function clanCards(items, limit=4){
  ...
  return `<div class="saas-clan-cards">${filtered.map((item,idx)=>{
    ...
    return `<article class="saas-clan-card">
      ...
      <div class="saas-clan-meta"><span>Membros reais <b>${memberCountValue(item)}</b></span><span>Dono <b>${buyerName(item)}</b></span><span>Contato <b>${escapeHtml(phoneText(item))}</b></span><span>E-mail <b>${escapeHtml(emailText(item))}</b></span><span>Expiração <b>${formatDate(item.planExpiresAt)}</b>${d !== null ? `<small>${d < 0 ? `há ${Math.abs(d)} dias` : `em ${d} dias`}</small>` : ""}</span></div>
