...
window.approveSubscriptionRequest = async id => {
  ...
  const plan = String(req.plan || "basic").toLowerCase();
  if(!clanTag){
    alert("Solicitação sem tag do clã.");
    return;
  }
...
function renderSales(){
  const list = document.querySelector("#saasUnifiedList");
  const pendingRequests = requestsCache.filter(item=>{
    const status = String(item.status || "pending").toLowerCase();
    return !["approved","archived","deleted","released"].includes(status);
  });
  if(!pendingRequests.length){
    list.innerHTML = `<div class="saas-row empty">Nenhuma venda/solicitação recebida.</div>`;
    return;
  }
  list.innerHTML = pendingRequests.map(item=>{
    const encoded = encodeURIComponent(JSON.stringify(item));
    const isPlanUpgrade = item.requestType === "planUpgrade";
    const releaseAction = isPlanUpgrade
      ? `approveSubscriptionRequest('${item.id}')`
      : `fillSaasReleaseFromRequest('${encoded}')`;
    const releaseLabel = isPlanUpgrade ? `Liberar ${planLabel(item.plan)}` : "Liberar";
    return `<article class="saas-row saas-request-row">
      <div class="saas-main-info"><strong>${isPlanUpgrade ? "Upgrade/Renovação" : (item.name||item.clanName||item.clanTag||"Solicitação")}</strong><span>${item.clanTag||"sem tag"} • ${item.buyerEmail||item.email||"sem email"}</span><small>${isPlanUpgrade ? `Solicitado: ${planLabel(item.plan)} ${item.price ? "• "+item.price : ""}` : "Nova venda/assinatura"}</small></div>
      <div class="saas-pill plan-${item.plan||"trial"}">${planLabel(item.plan)}</div>
      <div class="saas-actions"><button class="saas-small-action" onclick="${releaseAction}">${releaseLabel}</button><button class="saas-small-action danger" onclick="deleteSaasRequest('${item.id}')">Excluir</button></div>
    </article>`;
  }).join("");
}
