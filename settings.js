...
function renderApiInfo(){
  ...
  const memberCount=Number(currentPlanData.membersCount || currentPlanData.memberCount || 0) || "—";
  ...
  const rows=[
    ["Status", "Conectado"],
    ["Clã monitorado", clanTag],
    ["Seu vínculo", currentTag || "—"],
    ["Plano detectado", planLabel(currentPlan)],
    ["Membros detectados", memberCount],
    ["Última atualização", formatDateTime(updated)],
    ["Servidor", "Firestore + API Clash/VPS"],
    ["Modo", hasFullPermission()?"Admin/Liderança":"Membro"]
  ];
  grid.innerHTML=rows.map(([label,value])=>`<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("");
}
async function renderUsersList(){
  ...
  try{
    ...
    if(clanTag){
      ...
      users=snap.docs.map(d=>({id:d.id,...d.data()})).filter(u=>{
        const uClan=String(u.clanTag || u.currentClanTag || u.clan || "").replace("#","").toUpperCase();
        const c=String(clanTag||"").replace("#","").toUpperCase();
        return !uClan || !c || uClan===c || normalizeTag(u.playerTag||u.tag)===normalizeTag(getCurrentUserTag());
      });
    }
    const byTag=new Map(apiMembers.map(m=>[cleanTag(m.tag||m.playerTag||m.name),m]));
    const merged=[];
    users.forEach(u=>{
      const api=byTag.get(cleanTag(u.playerTag||u.tag||u.memberTag||u.name)) || {};
      merged.push({...api,...u, apiRole:api.role, apiNick:api.name});
    });
    apiMembers.forEach(m=>{
      const exists=merged.some(u=>cleanTag(u.playerTag||u.tag||u.memberTag||u.name)===cleanTag(m.tag||m.playerTag||m.name));
      if(!exists) merged.push({...m, apiRole:m.role, apiNick:m.name, systemOnly:false});
    });
    box.innerHTML=merged.length? merged.map(u=>{
      const role=u.apiRole || u.role || "member";
      const profile={...u, role};
      return `<article class="settings-user-row">
        <div class="settings-user-avatar">${escapeHtml(String(u.name||u.apiNick||u.displayName||"?").slice(0,1).toUpperCase())}</div>
        <div class="settings-user-main"><strong>${escapeHtml(u.nome||u.realName||u.name||u.apiNick||u.displayName||"Usuário")}</strong><span>${escapeHtml(u.apiNick||u.nick||u.displayName||u.name||"Nick não vinculado")} • ${escapeHtml(normalizeTag(u.playerTag||u.tag||u.memberTag||"sem tag"))}</span></div>
        <div class="settings-user-meta"><b>${escapeHtml(roleLabel(role))}</b><small>${escapeHtml(permissionLabel(profile))}</small></div>
      </article>`;
    }).join("") : `<div class="settings-popup-empty">Nenhum usuário cadastrado encontrado.</div>`;
  }catch(error){
    console.error(error);
...
async function registerCurrentSession(){
  const user=auth.currentUser; if(!user?.uid) return;
  const id=getSessionId();
  await setDoc(doc(db,"users",user.uid,"sessions",id),{device:sessionDeviceLabel(),current:true,status:"active",lastSeenAt:serverTimestamp(),userAgent:navigator.userAgent||""},{merge:true});
}
async function renderSessions(){
  const list=document.querySelector("#settingsSessionsList"); if(!list) return;
  const user=auth.currentUser;
  if(!user?.uid){ list.innerHTML=`<div class="settings-popup-empty">Usuário não autenticado.</div>`; return; }
  try{
    await registerCurrentSession();
    const current=getSessionId();
    const snap=await getDocs(collection(db,"users",user.uid,"sessions"));
    const sessions=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.id===current?-1:b.id===current?1:0));
    list.innerHTML=sessions.map(s=>`<article class="settings-session-row ${s.id===current?'current':''}"><div><strong>${escapeHtml(s.id===current?'Este dispositivo':s.device||'Dispositivo')}</strong><span>${escapeHtml(formatDateTime(s.lastSeenAt))}</span></div><em>${escapeHtml(s.status||'active')}</em>${s.id!==current?`<button type="button" data-end-session="${escapeHtml(s.id)}">Encerrar</button>`:''}</article>`).join("");
    list.querySelectorAll("[data-end-session]").forEach(btn=>btn.addEventListener("click",async()=>{
      await updateDoc(doc(db,"users",user.uid,"sessions",btn.dataset.endSession),{status:"ended",endedAt:serverTimestamp()});
      renderSessions();
    }));
  }catch(error){
    console.error(error);
    list.innerHTML=`<div class="settings-popup-empty">Sessões locais ativas. Para encerrar todas de forma imediata, precisaremos integrar com a VPS/backend.</div>`;
  }
}
