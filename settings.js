...
function hasRegistrationFields(u={}){
  return Boolean(
    u.linkedEmail || u.email || u.userEmail ||
    u.linkedUserUid || u.uid || u.userUid || u.linkedUid || u.userId ||
    u.linkedAt || u.registeredAt || u.createdAt || u.updatedAt ||
    u.nome || u.realName || u.displayName
  );
}
function mergeKey(u={}){
...
async function renderUsersList(){
  ...
  try{
    ...
    clanMemberDocs.filter(hasRegistrationFields).forEach(pushUser);
    // 2) Fonte recomendada daqui para frente: clans/{clanTag}/users/{uid}.
    clanUserDocs.filter(hasRegistrationFields).forEach(pushUser);
    ...
    box.innerHTML=merged.length? merged.map(u=>{
      ...
      return `<article class="settings-user-row">
        ...
        <div class="settings-user-main"><strong>${escapeHtml(realName)}</strong><span>${escapeHtml(nick)} • ${escapeHtml(tag)}${email?` • ${escapeHtml(email)}`:""}</span></div>
