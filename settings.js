...
function i18nS(key, values = {}){
  ...
  const fallback = {
    'pt-BR': {'settings.thisDevice':'Este dispositivo','settings.active':'ativa','settings.local':'local','settings.device':'Dispositivo','settings.ended':'encerrada','settings.endSession':'Encerrar','settings.syncing':'Em sincronização','settings.connected':'Conectado','settings.error':'Erro','settings.apiRefreshFailed':'Não foi possível atualizar a API agora.'},
    'en-US': {'settings.thisDevice':'This device','settings.active':'active','settings.local':'local','settings.device':'Device','settings.ended':'ended','settings.endSession':'End','settings.syncing':'Syncing','settings.connected':'Connected','settings.error':'Error','settings.apiRefreshFailed':'Could not refresh the API right now.'},
    'es-ES': {'settings.thisDevice':'Este dispositivo','settings.active':'activa','settings.local':'local','settings.device':'Dispositivo','settings.ended':'cerrada','settings.endSession':'Cerrar','settings.syncing':'Sincronizando','settings.connected':'Conectado','settings.error':'Error','settings.apiRefreshFailed':'No se pudo actualizar la API ahora.'}
  };
...
async function hydrateSettings(){
  ...
  const resolvedClanTag = currentPlanData.clanTag || currentPlanData.tag || clanData.clanTag || clanData.tag || getCurrentClanTag();
  if(resolvedClanTag){
    localStorage.setItem("topbrs_clan_tag", resolvedClanTag);
    localStorage.setItem("selectedClan", resolvedClanTag);
    localStorage.setItem("topbrs_clan", JSON.stringify({...clanData, ...currentPlanData, clanTag:resolvedClanTag, tag:resolvedClanTag}));
  }
...
async function refreshWholeSystemFromApi(){
  ...
  let ok=false;
  try{
    const activeClanTag = currentPlanData.clanTag || currentPlanData.tag || clanLocal.clanTag || clanLocal.tag || getCurrentClanTag();
    if(activeClanTag){
      localStorage.setItem('topbrs_clan_tag', activeClanTag);
      localStorage.setItem('selectedClan', activeClanTag);
    }
    if(btn){ btn.disabled=true; btn.textContent=i18nS('settings.syncing'); }
    const result = await syncClanAndMembersFromApi();
    if(result?.clan){
      currentPlanData = {...currentPlanData, ...result.clan, clanTag:result.clan.clanTag || result.clan.tag || activeClanTag, syncedAt:new Date().toISOString()};
      localStorage.setItem('topbrs_clan', JSON.stringify(currentPlanData));
    }
    ...
    console.error('Refresh API/VPS falhou:', error);
    currentPlanData = {...currentPlanData, lastError:String(error?.message || error), apiError:true};
    await renderApiInfo();
    alert(i18nS('settings.apiRefreshFailed') || 'Não foi possível atualizar a API agora.');
  }finally{
