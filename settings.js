...
function updatePaymentStatus(){
  ...
  const completed = status.status === "approved" || (requestedPlan && requestedPlan !== "trial" && normalizePlan(currentPlan) === requestedPlan);
  row.classList.toggle("complete", completed);
  row.classList.toggle("pending", !completed);
  icon.src = completed ? "assets/icons/check-complete.svg" : "assets/icons/history-clock.svg";
  icon.alt = completed ? "Concluído" : "Pendente";
  const label = row.querySelector("span");
  if(label){
    label.textContent = completed
      ? `Plano ${planLabel(requestedPlan || currentPlan)} liberado no SaaS`
      : (status.plan ? `Aguardando liberação do ${planLabel(status.plan)} no SaaS` : "3. Aguarde liberação no SaaS");
  }
}
...
async function requestClanPlan(){
  ...
  try{
    ...
    updatePaymentStatus();
    if(feedback){ feedback.textContent = `Solicitação do plano ${planLabel(plan)} enviada ao Painel SaaS.`; feedback.className = "tag-feedback success"; }
  }catch(error){
    console.error(error);
    if(feedback){ feedback.textContent = "Erro ao enviar solicitação. Verifique as regras do Firestore."; feedback.className = "tag-feedback error"; }
  }
}
