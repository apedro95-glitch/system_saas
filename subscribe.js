...
function updateStepPills(){
  ...
  if(planPill) planPill.textContent = `${PLAN_LABELS[plan]} selecionado`;
  if(cyclePill) cyclePill.textContent = `${CYCLE_LABELS[cycle]} • ${money(amount)}`;
  if(dataPill) dataPill.textContent = "Preencher dados";
  if(planHint) planHint.textContent = "Plano escolhido. Toque na etapa se quiser trocar.";
  if(cycleHint) cycleHint.textContent = "Período escolhido. Toque na etapa se quiser trocar.";
  if(dataHint) dataHint.textContent = "Última etapa antes de criar o pedido pendente.";
}
