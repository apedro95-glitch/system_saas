...
export async function fetchRiverRaceFromApi(clanTag){
  ...
  const clean = cleanTag(clanTag);
  const encodedHash = `%23${encodeURIComponent(clean)}`;
  const endpoints = [
    `${apiBaseUrl}/api/clan/${encodeURIComponent(clean)}/riverrace`,
    `${apiBaseUrl}/api/clan/${encodedHash}/riverrace`,
    `${apiBaseUrl}/api/clan/${encodeURIComponent(clean)}/war`,
    `${apiBaseUrl}/api/clan/${encodedHash}/war`,
    `${apiBaseUrl}/api/clan/${encodeURIComponent(clean)}/currentriverrace`,
    `${apiBaseUrl}/api/clan/${encodedHash}/currentriverrace`
  ];
  ...
  for(const url of endpoints){
    try{
      ...
      if(data?.ok && (data.riverRace || data.war || data.currentRiverRace || data.data)){
        return data.riverRace || data.war || data.currentRiverRace || data.data;
      }
      if(data?.clan?.participants || data?.participants || data?.clanParticipants || data?.currentRiverRace?.clan?.participants){
        return data;
      }
    }catch(error){
...
function normalizeParticipant(raw){
  const attacksUsed =
    Number(raw.decksUsedToday ?? raw.decksUsedTodayCount ?? raw.attacksUsedToday ?? raw.boatAttacksToday ?? raw.attacksToday ?? raw.attacks ?? 0);

  const weeklyAttacks =
    Number(raw.decksUsed ?? raw.decksUsedThisWeek ?? raw.attacksUsed ?? raw.totalAttacks ?? raw.totalDecksUsed ?? attacksUsed);
