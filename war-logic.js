...
export async function syncWarSilently(){
  ...
  if(ctx.active && ctx.dayKey){
    for(const p of participants){
      ...
      const previousDaysFame = WAR_DAYS
        .filter(dayKey=>dayKey !== ctx.dayKey)
        .reduce((sum,dayKey)=>sum + Number(membersMap[id].days?.[dayKey]?.fame || 0),0);

      let currentDayFame;
      if(p.dailyFame !== null && p.dailyFame !== undefined){
        currentDayFame = p.dailyFame;
      }else{
        currentDayFame = Math.max(0, Number(p.weeklyFame || p.fame || 0) - previousDaysFame);
      }

      membersMap[id].days[ctx.dayKey] = {
        attacks: attacksToday,
        fame: Math.max(Number(membersMap[id].days?.[ctx.dayKey]?.fame || 0), currentDayFame),
      };

      membersMap[id].weeklyFame = Math.max(
        Number(p.weeklyFame || p.fame || 0),
        sumDaysFame(membersMap[id].days),
        Number(membersMap[id].weeklyFame || 0)
      );

      // Se a soma dos dias ficou maior que a pontuação semanal de API por causa de histórico antigo,
      // a UI semanal passa a usar weeklyFame, e os dias são normalizados para não inflar a semana.
      membersMap[id].days = normalizeStoredDailyFame(membersMap[id].days, membersMap[id].weeklyFame);
      if(Number(p.weeklyFame || p.fame || 0) > 0){
        membersMap[id].weeklyFame = Number(p.weeklyFame || p.fame || 0);
      }
  ...
  for(const member of Object.values(membersMap)){
    ...
    member.weeklyFame = Math.max(Number(member.weeklyFame || 0), fameByDays);
