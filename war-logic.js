...
function normalizeParticipant(raw){
  const attacksToday =
    Number(raw.decksUsedToday ?? raw.attacksUsedToday ?? raw.boatAttacksToday ?? raw.attacksToday ?? 0);

  const weeklyAttacks =
    Number(raw.decksUsed ?? raw.attacksUsed ?? raw.totalAttacks ?? raw.decksUsedTotal ?? attacksToday);

  const fame =
    Number(raw.fame ?? raw.repairPoints ?? raw.boatPoints ?? raw.clanScore ?? 0);
