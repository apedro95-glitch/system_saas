...
function monthShortFromMonthKey(monthKey){
 const monthNumber = Number(String(monthKey || '').split('-')[1]);
 return monthOrder[Math.max(0, Math.min(11, (monthNumber || 1) - 1))] || 'Jan';
}
...
async function loadRealWarMembers(){
 const ctx=getWarWeekContext(new Date());
 const ctxMonthShort = monthShortFromMonthKey(ctx.monthKey);
 CURRENT_MONTH=ctxMonthShort; CURRENT_WEEK=ctx.weekKey; selectedMonth=ctxMonthShort; selectedWeek=ctx.weekKey;
 ...
 warMembers=apiMembers.map(m=>{
  ...
  const weeklyAttacks=saved.weeklyAttacks || 0;
  return {
   ...
   fame:['D1','D2','D3','D4'].reduce((sum,k)=>sum + Number(saved.days?.[k]?.fame || 0),0),
   dayFame:Number(saved.days?.[['D1','D2','D3','D4'][selectedDay] || ctx.dayKey || 'D1']?.fame || 0),
   dayAttacks:getDailyAttackDots(saved, ['D1','D2','D3','D4'][selectedDay] || ctx.dayKey || 'D1'),
   days:saved.days || {D1:{attacks:0},D2:{attacks:0},D3:{attacks:0},D4:{attacks:0}},
...
window.topbrsAfterRefresh = async function(){
  await loadRealWarMembers();
  renderWarList();
};
