...
function renderWarList(){
 ...
 if(activeWarTab==='members'){
  ...
  list.innerHTML=sortWarMembersForActiveTab(warMembers).map((m,i)=>`<div class="war-row members-row ${m.current ? 'is-current-member' : ''}"><div class="war-member-cell"><span class="war-avatar tier-${Math.min(i+1,4)}"><img src="${m.avatarSrc || 'assets/icons/profile-user.svg'}" alt="" aria-hidden="true"></span><strong>${m.name}${m.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong></div><strong class="fame ${fameClass(m.fame)}">${fmt(m.weeklyFame ?? m.fame)}</strong><strong class="attack-count">${m.weekly}</strong></div>`).join('');
 }else{
  head.innerHTML='<span>Membro</span><span>Fame</span><span>Ataques</span>';
  list.innerHTML=sortWarMembersForActiveTab(warMembers).map((m,i)=>`<div class="war-row attacks-row ${m.current ? 'is-current-member' : ''}"><div class="war-member-cell"><span class="war-avatar tier-${Math.min(i+1,4)}"><img src="${m.avatarSrc || 'assets/icons/profile-user.svg'}" alt="" aria-hidden="true"></span><strong>${m.name}${m.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong></div><strong class="day-fame ${fameClass(m.days?.[['D1','D2','D3','D4'][selectedDay]]?.fame ?? m.dayFame ?? 0)}">${fmt(m.days?.[['D1','D2','D3','D4'][selectedDay]]?.fame ?? m.dayFame ?? 0)}</strong>${dots(m.days?.[['D1','D2','D3','D4'][selectedDay]]?.attacks || m.dayAttacks || 0)}</div>`).join('');
 }
}
