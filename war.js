import { loadMembers, periodLabelNow, warWindowState } from './real-data.js';
...
function updateWarStatus(){
 ...
 if(state==='finished'){
  ...
  if(label) label.textContent=warWindowState().active?'EM ANDAMENTO':'FORA DA JANELA';
 }
}
...
function updateWarBlock(){
 ...
 const totalAttacks=warMembers.length*WEEKLY_ATTACKS_PER_MEMBER;
...
function renderWarList(){
 ...
 if(activeWarTab==='members'){
  ...
  list.innerHTML=warMembers.map((m,i)=>`<div class="war-row members-row"><div class="war-member-cell"><span class="war-avatar tier-${Math.min(i+1,4)}"><img src="${m.avatarSrc || 'assets/icons/profile-user.svg'}" alt="" aria-hidden="true"></span><strong>${m.name}${m.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong></div><strong class="attack-count">${m.weekly}</strong><strong class="fame ${fameClass(m.fame)}">${fmt(m.fame)}</strong></div>`).join('');
 }else{
  head.innerHTML='<span>Membro</span><span>Fame</span><span>Hoje</span>';
  list.innerHTML=warMembers.map((m,i)=>`<div class="war-row attacks-row"><div class="war-member-cell"><span class="war-avatar tier-${Math.min(i+1,4)}"><img src="${m.avatarSrc || 'assets/icons/profile-user.svg'}" alt="" aria-hidden="true"></span><strong>${m.name}${m.current ? ' <em class="you-badge">VOCÊ</em>' : ''}</strong></div><strong class="day-fame ${fameClass(m.dayFame*4)}">${fmt(m.dayFame)}</strong>${dots(m.dayAttacks)}</div>`).join('');
 }
}
...
async function loadRealWarMembers(){
 ...
 CURRENT_MONTH=p.short; CURRENT_WEEK=p.weekCode; selectedMonth=p.short; selectedWeek=p.weekCode;
 warMembers=(await loadMembers()).filter(m=>!m.removed).map(m=>({
  name:m.name||'Membro',
